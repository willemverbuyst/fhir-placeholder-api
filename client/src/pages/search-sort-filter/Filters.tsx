...
export function Filters<T>(props: Props<T>): React.JSX.Element {
  ...
  return (
    <section className="flex flex-col gap-6 items-start w-[350px]">
      {(Object.entries(filterKeys) as [string, Set<string | boolean>][]).map(
        ([key, v]) => (
          <div key={key} className="flex flex-col gap-2 items-start w-[350px]">
            <h3 className="text-xl">{key}</h3>
            <section className="grid grid-cols-2 space-x-6">
