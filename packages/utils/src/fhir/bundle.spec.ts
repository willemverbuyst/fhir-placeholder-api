...
import { getResourcesWithIdFromBundle, isBundle } from "./bundle";
...
describe("getResourcesWithIdFromBundle", () => {
  it("returns an array of resources from a valid Bundle", () => {
    ...
    const result = getResourcesWithIdFromBundle(bundle);
  ...
  it("returns an empty array if bundle is undefined", () => {
    expect(getResourcesWithIdFromBundle(undefined)).toEqual([]);
  });

  it("returns an empty array if bundle is not a Bundle", () => {
    const resource = { resourceType: "Patient", id: "1" };
    // @ts-ignore -force passing a non-bundle object
    expect(getResourcesWithIdFromBundle(resource)).toEqual([]);
  });
  ...
  it("returns an empty array if bundle.entry is undefined", () => {
    ...
    expect(getResourcesWithIdFromBundle(bundle)).toEqual([]);
  });
